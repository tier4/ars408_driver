...
void Ars408Driver::UpdateObjectQuality(
  ...
  const ars408::Obj_2_Quality & in_object_quality)
{
...
void Ars408Driver::ParseRadarState(const std::array<uint8_t, 8> & in_can_data)
{
  ...
  current_radar_state_.SortingMode = ars408::RadarState::SortingConfig(
    (in_can_data[4] & 0x70u) >> 4u);
  current_radar_state_.PowerMode = ars408::RadarState::PowerConfig(
    (in_can_data[3] << 1u) +
    ((in_can_data[4] & 0x80u ) >> 7u));
  current_radar_state_.EgoMotionRxStatus = ars408::RadarState::MotionRx(
    (in_can_data[5] & 0xC0u) >> 6u);
  ...
  current_radar_state_.OutputType = ars408::RadarState::OutputTypeConfig(
    (in_can_data[5] & 0x0Cu) >> 2u);
